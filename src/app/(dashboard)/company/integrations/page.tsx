...
export default async function CompanyIntegrationsPage() {
  ...
  // Declared once and placed twice — see the note on the branch below.
  const listColumn = (
      ...
      <Card>
        <CardHeader>
          ...
          {/* `<p className="mt-1 text-sm text-muted-foreground">` is
                ...
                on the page). */}
          <CardDescription>
            Every hourly refresh, manual refresh and spreadsheet upload, newest first — so you can
            see what came in and what went wrong.
          </CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          {jobs.length === 0 ? (
  ...
  const addColumn = (className: string) => (
    <div className={className}>
      <Card id="connect-integration">
        <CardHeader>
          <CardTitle>Connect an integration</CardTitle>
          <CardDescription>
            Google Calendar signs you in and needs nothing else. Everything else asks for an address
            and a key from the shop you are linking.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button asChild variant="outline">
              ...
              <a href="/api/integrations/custom/schema" target="_blank" rel="noopener noreferrer">
                Custom API schema
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      {/*
        ...
        sticky, because you fill the form in while reading the list beside it.

        WITH NOTHING CONNECTED: no split, and the order flips. That 1.6fr column
        ...
        beside it, while the form itself was squeezed into the remaining ~400px
        third. The two ways to add something ARE the task on a fresh account, so
        they take the width and come first. Same reasoning `/company/flows` was
        rebuilt on.
        ...
        screen-reader user would be read "Nothing connected yet — use the form
        above" before reaching a form that is, in their order, below.

        Both columns are declared once, above, and placed twice. Inlining them
        into each branch would have duplicated 190 lines of cards and tables,
        which is how the two halves of a conditional layout drift apart.
      */}
      {hasIntegrations ? (
        // `[&>*]:min-w-0`: a grid item's default `min-width: auto` is "as wide
        // as my longest unbreakable child", and this column holds tables of
        // webhook URLs and provider error messages.
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)] [&>*]:min-w-0">
          {listColumn}
          {addColumn('space-y-6 lg:sticky lg:top-6 lg:self-start')}
        </div>
      ) : (
        <div className="space-y-6">
          {/* Not sticky: on a fresh account this IS the page, and a sticky
              element taller than the viewport simply stops scrolling. Side by
              side at a floor rather than a breakpoint. */}
          {addColumn(
